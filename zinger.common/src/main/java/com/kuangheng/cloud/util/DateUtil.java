...
public class DateUtil {
    ...
    public static Date formatString(String target, String format) {
    ...
    public static String createCron(String cronExpress){
        if (StringUtils.isBlank(cronExpress)){
            return "";
        }
        String[] cronExpressStr = cronExpress.split("&");
        String one = cronExpressStr[0];
        String two = cronExpressStr[1];
        // 具体的时间 12:10:00
        String three = cronExpressStr[2];
        String[] time = three.split(":");
        String cron = time[2] + " " + time[1] + " " + time[0] + " ";
        if ("1".equals(one)){
            // 选择的是每月，传过来的数字就是每月的多少号
            cron += two + " " + "* ?";
        }else if ("2".equals(one)){
            // 选择的是每周，传过来的数字就是周几，例如周一就是MON, SUN, MON, TUE, WED, THU, FRI, SAT
            cron += "? * " + two;
